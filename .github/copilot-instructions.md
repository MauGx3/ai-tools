...
- **Jekyll Site with Collections**: Static site using Just the Docs theme; content organized into collections (`_prompts/`, `_instructions/`, `_modes/`, `_thoughts/`) with YAML front matter for metadata and layouts (see `_config.yml` for collection configs).
- **Diátaxis Documentation**: Docs under `docs/` follow tutorial/how-to/reference/explanation structure (e.g., `docs/tutorials/getting-started.md` for onboarding, `docs/reference/repo-structure.md` for quick refs).
- **Memory Bank Persistence**: `memory-bank/` stores canonical context (projectbrief.md, activeContext.md, progress.md) to overcome stateless AI limitations—update after significant changes.
- **Automation Scripts**: Python utilities in `scripts/` (e.g., `scan_starred_repos.py` uses GitHub API with dotenv for env vars; outputs to `results/`).
- **Why This Structure**: Enables reproducible AI workflows; content-first approach avoids monolithic apps; Jekyll handles static deployment to GitHub Pages.

## Workflows

- **Local Development**: `bundle exec jekyll serve` for site preview (requires Ruby/Bundler); Python scripts use venv (`python -m venv .venv; source .venv/bin/activate.fish; pip install -r scripts/requirements.txt`).
- **Script Execution**: Run with env vars (e.g., `GITHUB_TOKEN` for API access); outputs JSON/markdown to `results/` or `data/`.
- **Deployment**: Automatic via GitHub Pages; no manual build steps beyond commits.
- **Testing**: Playwright for site E2E (`playwright.config.ts`); no unit tests for scripts yet—validate manually.
- **Debugging**: Check Jekyll logs for site issues; script errors via stdout/stderr; use `python -m pdb` for breakpoints.

## Conventions

- **applyTo Scoping**: Instruction files use `applyTo` globs (e.g., `scripts/*.py`) to target specific files—audit for dead patterns after refactors.
- **Conventional Commits**: Enforce `<type>[scope]: <description>` (e.g., `feat(docs): add tutorial page`); scopes like `docs(memory-bank)` pinpoint areas.
- **Security-First**: Follow `security-and-owasp.instructions.md`; no hardcoded secrets; use env vars; validate inputs (e.g., in scripts, sanitize API responses).
- **Content Patterns**: Prompts include purpose/audience/examples; instructions have decision criteria; modes specify personas; thoughts are append-only experiments.
- **Memory Bank Updates**: Log changes in `activeContext.md` and `progress.md`; create task files in `tasks/` for tracking (e.g., `TASK001-initialize-memory-bank.md`).
